...
void printEntry(cm3l_MapEntry ent) {
	void *keyaddr = CM3L_KEYADDR_SELECT(ent.key);
	if (keyaddr == NULL) {
		puts("<null>");
		return;
	}
	...
	printf("\"]: %d <ks: %d, vs: %d>\n", ent.value ? *(int*)ent.value : -1, (int)ent.key.size, (int)ent.valsize);
}
...
int main(int argc, char **argv)
{
	...
	for (;;)
	{
		...
		if (!strcmp(cmds, "i") || !strcmp(cmds, "ins"))
		{
			...
			ent = cm3l_MapEntryCreateK(key, strlen(key), &value, sizeof(int));
		...
		else if (!strcmp(cmds, "g") || !strcmp(cmds, "get"))
		{
			...
			cm3l_MapEntry *f = cm3l_HashMapAccessK(&m, key, strlen(key));
			ent = f ? *f : (cm3l_MapEntry) {{0, {.ptr = NULL}}, 0, NULL};
			printEntry(ent);
		}
		else if (!strcmp(cmds, "e") || !strcmp(cmds, "erase"))
		{
			...
			fscanf(fp, "%s", key);
			if (echo == 2) printf("%s %s\n", cmds, key);
			int erased = cm3l_HashMapEraseK(&m, key, strlen(key));
		...
		else if (!strcmp(cmds, "a") || !strcmp(cmds, "all"))
		{
			...
			for (;;)
			{
				...
				cm3l_MapEntry *ent = cm3l_HashMapIterAccess(&iter);
				printEntry(*ent);
			}
		}
		...
		else if (!strcmp(cmds, "as") || !strcmp(cmds, "assert"))
		{
			...
			int fval = (f && f->valsize == sizeof(int) && f->value) ? *(int*)f->value : 0;
			...
			if (notNull)
			{
				...
				else if (f->valsize != sizeof(int))
					printf("[assertion failed: %d != undefined]\n", cval);
				else if (cval != fval)
			...
			{
				++aerr;
				if (f->valsize != sizeof(int))
					printf("[assertion failed: null != undefined]\n");
				else printf("[assertion failed: null != %d]\n", fval);
			}
		}
